"""CDD-only verification — turn HS-KD off (λ_hs = 0) but keep CDD on.

Tests the hypothesis that HS-KD and CDD are antagonistic (HS-KD pushes the
student toward `pre`, while CDD pushes it along `pre - post`). If CDD has
independent merit, removing HS-KD should let it shine.

Effective loss for these 3 runs:
    L = L_rec + λ_pred · L_pred + 0 · L_hs + λ_cdd · L_cdd

To still route through KDStudentDistillTrainer (which owns the CDD code), we
keep --do_hs_distill and just zero out --lambda_hs.

Settings (3 runs):
    model_type     = kdstudent
    λ_pred, T      = per-DS v1 grid winner
    λ_hs           = 0.0           (HS-KD disabled)
    λ_cdd, α       = 0.05, 0.5     (same as run_cdd_initial.py)

Usage:
    python run_cdd_only.py
    python run_cdd_only.py --datasets Beauty
    python run_cdd_only.py --dry_run
    python run_cdd_only.py --skip_existing
"""
import argparse
import os
import subprocess
...
# Pred-KD HP from v1 winners (λ_hs forced to 0 below)
BEST_HP = {
    "Beauty": {"lp": "2.0", "T": "1.0", "layer": "last"},
    "LastFM": {"lp": "2.0", "T": "2.0", "layer": "last"},
    "ML-1M":  {"lp": "0.5", "T": "5.0", "layer": "last"},
}
...
def build_cmd(ds, args):
    ...
    train_name = f"kdstudent_cdd_only_{ds}_initial"
    ...
    cmd = [
        ...
        "--lambda_kd", hp["lp"],
        "--kd_temperature", hp["T"],
        # Keep --do_hs_distill so the dispatch picks KDStudentDistillTrainer,
        # but zero λ_hs so HS-KD contributes nothing.
        "--do_hs_distill",
        "--lambda_hs", "0.0",
        "--hs_loss_type", "mse",
...
def main():
    ...
    print(f"[cdd-only] planned {len(args.datasets)} runs "
          f"(λ_hs=0, λ_cdd={LAMBDA_CDD}, α={CDD_ALPHA})")
    ...
    print(f"\n[cdd-only] total wall time: {time.perf_counter() - t0:.1f}s")
